...
mkdir -p "$HOME/public_html/npdemo4" && cp -r "$NP_SCRIPT_DIR/working_dir/"* "$HOME/public_html/npdemo4"
...
if cd "$DEMO_DIR"; then
  ...
  if ! (cp src/console.cgi "$HOME/public_html/npdemo4"); then
    ERROR "Cannot copy your console.cgi to ~/public_html/npdemo4 (Did you name it right?)."
    exit 1
  fi
  ...
  echo "      http://$(hostname).cs.nycu.edu.tw:$HTTP_SERVER_PORT/printenv.cgi?course_name=NP"
  echo "      http://$(hostname).cs.nycu.edu.tw:$HTTP_SERVER_PORT/hello.cgi"
  echo "      http://$(hostname).cs.nycu.edu.tw:$HTTP_SERVER_PORT/welcome.cgi"
  echo ""
  INFO "Test console.cgi"
  echo "      http://$(hostname).cs.nycu.edu.tw/~$USER_NAME/npdemo4/panel.cgi"
  echo ""
